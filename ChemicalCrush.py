...
def Line_art(): # Função para exibição de uma arte com o nome do jogo
    art = [
        "  ***   *  *   |***  ***   ***  *   ***    ***    *             ***   *  *   *  *  ****   *   *",
        " *      ****   |***  *  * *  *  *  *      * * *   *            *      ****   *  *    \    **** ",
        "  ***   *  *   |***  *   *   *  *   ***  *     *  ****          ***   *  *   ****  ****   *   *",
...
def Efeito_Point(pontuacao): # Funcao para avisar o jogador que ele fez um ponto
    Clear()
...
def InGame(elementos, pontuacao):   # Funcao principal do jogo
    Clear()
    ...
    guiaLinha = 1
    for i in elementos:
        print(guiaLinha, "|", end="")   # exibição de um guia para o jogador selecionar uma linha na matriz
        for j in i:
            print(" " * 5, j, end=" ")  # exibição da matriz onde está os elementos quimicos
        if (guiaLinha == 1):
        ...
        print("\n")
    time.sleep(3)
    while (Point(elementos) == 1 and pontuacao < 3):    # caso o jogador faça um ponto é possivel que os  novos elementos a serem inseridos também faça pontos.
        pontuacao += 1
...
def main(): # função inicial para iniciar o jogo
    Clear()
